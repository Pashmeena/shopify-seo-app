...
| Transient provider failures | Separate from content retries: rate limits (429), server errors (5xx/529), timeouts and network drops are retried twice with backoff (1 s, 4 s) at the shared client layer — uniformly for all three providers. Permanent errors (bad key, bad request, refusal) fail immediately. | A momentary provider hiccup shouldn't fail a generation; a wrong API key shouldn't waste 5 s pretending it might. |
