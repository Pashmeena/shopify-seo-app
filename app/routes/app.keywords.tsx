...
import { generatePageForKeyword, PipelineRejection } from "../services/plp/pipeline.server";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const [keywords, settings] = await Promise.all([listKeywords(shop), getSettings(shop)]);
    ...
    .map((line) => {
      ...
      if (lastComma > 0) {
        ...
        if (validLocales.has(maybeLocale)) {
          return { phrase: line.slice(0, lastComma).trim(), locale: maybeLocale };
        }
      }
  ...
  try {
    switch (intent) {
      case "add": {
        ...
        const fallbackLocale = String(formData.get("locale") || settings.defaultLocale);

        const pasted = String(formData.get("keywords") || "");
        const file = formData.get("csv");
        const uploaded = file instanceof File && file.size > 0 ? await file.text() : "";

        const entries = parseKeywordLines(`${pasted}\n${uploaded}`, fallbackLocale, validLocales);
        if (entries.length === 0) return { error: "No keywords found in the input." };
        ...
        return { success: `Added ${created} keyword(s) (${entries.length - created} already known).` };
      }
      case "discover": {
      ...
      case "reject": {
        ...
        return { success: intent === "approve" ? "Keyword approved." : "Keyword rejected." };
      }
      case "generate": {
        const id = String(formData.get("keywordId"));
...
export default function Keywords() {
  const { keywords, locales, defaultLocale, minProducts, ai } = useLoaderData<typeof loader>();
  ...
  const busyKeywordId =
    navigation.state === "submitting" ? String(navigation.formData?.get("keywordId")) : null;
          ...
          <BlockStack gap="400">
            ...
            <Form method="post" encType="multipart/form-data">
              ...
              <FormLayout>
                ...
                <InlineStack gap="400" blockAlign="end" wrap>
                  ...
                  <Button submit variant="primary" loading={busyAction === "add"}>
                    Add & parse intent
                  </Button>
                </InlineStack>
              </FormLayout>
            ...
            <InlineStack gap="200" blockAlign="center">
              ...
              <Text as="span" tone="subdued" variant="bodySm">
                Product-first: only facet combinations with ≥ {minProducts} matching products are
                suggested. Nothing publishes without your approval.
              </Text>
            </InlineStack>
          </BlockStack>
        </Card>
        ...
        <Card padding="0">
          ...
          >
            {keywords.map((keyword, index) => (
              <IndexTable.Row id={keyword.id} key={keyword.id} position={index}>
                <IndexTable.Cell>
                  <BlockStack gap="050">
                ...
                <IndexTable.Cell>
                  {keyword.matchCount == null ? (
                    "—"
                  ) : (
                    <Text
                      as="span"
                      tone={keyword.matchCount < minProducts ? "critical" : "success"}
                    >
                      {keyword.matchCount}
                    </Text>
                  )}
                ...
                <IndexTable.Cell>
                  <InlineStack gap="100">
                    {(keyword.status === "suggested" || keyword.status === "rejected") && (
                      <Form method="post">
                        ...
                        <Button
                          submit
                          size="micro"
                          loading={busyAction === "approve" && busyKeywordId === keyword.id}
                        >
                          Approve
                        </Button>
                      </Form>
                    )}
                    {(keyword.status === "suggested" || keyword.status === "approved") && (
                      <Form method="post">
                        ...
                        <Button
                          ...
                          size="micro"
                          tone="critical"
                          loading={busyAction === "reject" && busyKeywordId === keyword.id}
                        >
                          Reject
                        </Button>
                      </Form>
                    )}
                    {(keyword.status === "approved" || keyword.status === "failed") && (
                      <Form method="post">
                        ...
                        <Button
                          ...
                          size="micro"
                          variant="primary"
                          loading={busyAction === "generate" && busyKeywordId === keyword.id}
                        >
                          Generate PLP
                        </Button>
                      </Form>
